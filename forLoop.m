...
for effectIndex = 1:totalPoints

    % Wait until this channel profile timestamp
    pause(max(0, channelProfile(effectIndex,1) - toc(loopTimer)));
    ...
    % Apply Doppler + delay
    [phaseOffset, circBuffer, writePointer, tx_data] = applyDigitalImpairments(rx_data, current_fShift, phaseOffset, current_delay, ...
        circBuffer, writePointer, SamplesPerFrame, fs);

    % Transmit
    SDR_TX(tx_data);

    % Prevent invalid attenuation
    current_db = max(0,current_db);

    fprintf("Time: %.2fs | Total Atten: %.2f dB | Path Loss: %.2f dB | Delay: %.2f ms | Doppler: %.2f Hz\n", ...
        channelProfile(effectIndex,1), current_db, current_pathloss, current_delay*1e3, current_fShift);
