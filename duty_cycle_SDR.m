...
channelProfile(:,2) = 130*ones(size(csv_table{:, 5}));                  
...
% channelProfile(1:19,3) = 0;                                              % Enable for Circular Buffer Delay Testing           
% channelProfile(20:end,3) = 1.15e-6;                                         % Enable for Circular Buffer Delay Testing
...
%% ---------------- Duty Cycle Measurement Setup ----------------
% "On" time = wall-clock time spent inside SDR_RX() + SDR_TX() calls per loop
% iteration. "Off" time = the remainder of the iteration (impairment math,
% release()/retrigger overhead, plotting, attenuator writes). Logged per
% RF frame (SamplesPerFrame samples), not per CSV row, since the RX/TX
% loop runs much faster than the 1-row-per-second channel profile updates.
framesPerSec_est = fs / SamplesPerFrame;                          % Approx. RX/TX frames per second
dutyLogCapacity  = ceil(totalPoints * framesPerSec_est * 1.5) + 200;  % Preallocate w/ margin
onTimeLog        = NaN(dutyLogCapacity, 1);
totalTimeLog     = NaN(dutyLogCapacity, 1);
dutyCycleLog     = NaN(dutyLogCapacity, 1);
frameCount       = 0;
dutyClock        = tic;   % Master reference clock for all duty-cycle timestamps
%% ----------------------------------------------------------------
...
while (effectIndex <= totalPoints)
    ...
    frameCount = frameCount + 1;               %#ok<*NASGU>
    iterStart  = toc(dutyClock);                % Start of this loop iteration
    
    % Pull a live RF data frame from the USRP Receiver
    SDR_on = tic %%%%%%%%%%%%%%%%%%%%
    tRxStart = toc(dutyClock);                  % --- duty cycle: RX on ---
    rx_data = SDR_RX();
    tRxEnd = toc(dutyClock);                    % --- duty cycle: RX off ---

    % Extract current parameters from processed profile matrix
    current_time      = getRadioTime(SDR_RX)
    % current_time      = channelProfile(effectIndex, 1);%%%%%%%%%%%%%%%%%%%%
    ...
    triggerTime       = current_time + current_delay;%%%%%%%%%%%%%%%%%%%%
    ...
    tTxStart = toc(dutyClock);                  % --- duty cycle: TX on ---
    SDR_TX(tx_data);
    tTxEnd = toc(dutyClock);                    % --- duty cycle: TX off ---
    toc(SDR_on)%%%%%%%%%%%%%%%%%%%%

    %% ---------------- Duty Cycle Logging ----------------
    iterEnd       = toc(dutyClock);
    onDuration    = (tRxEnd - tRxStart) + (tTxEnd - tTxStart);
    totalDuration = iterEnd - iterStart;
    if frameCount <= dutyLogCapacity
        onTimeLog(frameCount)    = onDuration;
        totalTimeLog(frameCount) = totalDuration;
        dutyCycleLog(frameCount) = onDuration / totalDuration;
    else
        warning('Duty cycle log capacity exceeded; extend dutyLogCapacity for full-run logging.');
    end
    %% ------------------------------------------------------
    ...
    if (channelProfile(effectIndex, 1) <= toc(loopTimer))
        ...
        % Report duty cycle over the most recent second of frames
        recentIdx = max(1, frameCount - round(framesPerSec_est)) : frameCount;
        recentIdx = recentIdx(recentIdx >= 1 & recentIdx <= dutyLogCapacity);
        fprintf("  -> SDR Duty Cycle (last ~1s, %d frames): %.2f %% (on: %.3f ms / iter: %.3f ms avg)\n", ...
            numel(recentIdx), mean(dutyCycleLog(recentIdx), 'omitnan')*100, ...
            mean(onTimeLog(recentIdx), 'omitnan')*1e3, mean(totalTimeLog(recentIdx), 'omitnan')*1e3);
...
%% ---------------- Duty Cycle Summary & Export ----------------
validIdx = 1:min(frameCount, dutyLogCapacity);
if ~isempty(validIdx)
    meanDuty = mean(dutyCycleLog(validIdx), 'omitnan');
    minDuty  = min(dutyCycleLog(validIdx));
    maxDuty  = max(dutyCycleLog(validIdx));

    fprintf('\n--- SDR Duty Cycle Summary ---\n');
    fprintf('RF frames measured : %d\n', numel(validIdx));
    fprintf('Mean duty cycle    : %.2f %%\n', meanDuty*100);
    fprintf('Min duty cycle     : %.2f %%\n', minDuty*100);
    fprintf('Max duty cycle     : %.2f %%\n', maxDuty*100);
    fprintf('Mean iteration time: %.3f ms (on: %.3f ms, off: %.3f ms)\n', ...
        mean(totalTimeLog(validIdx), 'omitnan')*1e3, ...
        mean(onTimeLog(validIdx), 'omitnan')*1e3, ...
        (mean(totalTimeLog(validIdx), 'omitnan') - mean(onTimeLog(validIdx), 'omitnan'))*1e3);

    % Static post-run duty cycle plot
    dutyFig = figure('Name', 'SDR Duty Cycle Over Run');
    plot(validIdx, dutyCycleLog(validIdx) * 100, 'LineWidth', 1.2);
    xlabel('RF Frame Index'); ylabel('Duty Cycle (%)');
    title('SDR On/Off Duty Cycle per RX/TX Loop Iteration'); grid on;
    yline(meanDuty*100, '--r', sprintf('Mean = %.1f%%', meanDuty*100));

    % Export raw duty cycle data alongside the source CSV for later analysis
    dutyCycleResults = table((1:numel(validIdx))', onTimeLog(validIdx)*1e3, ...
        totalTimeLog(validIdx)*1e3, dutyCycleLog(validIdx)*100, ...
        'VariableNames', {'FrameIndex','OnTime_ms','IterationTime_ms','DutyCycle_pct'});
    dutyOutFile = fullfile(path, "dutyCycle_" + erase(csv_filename, ".csv") + ".csv");
    writetable(dutyCycleResults, dutyOutFile);
    fprintf('Duty cycle log written to: %s\n', dutyOutFile);
else
    fprintf('No duty cycle frames were logged.\n');
end
%% ----------------------------------------------------------------
